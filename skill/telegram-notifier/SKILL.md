...
- If `skill/telegram-notifier/config.local.env` is missing or incomplete, read `references/setup-telegram.md` and run the setup flow first.

## Workflow

1. Confirm the task is truly finished.
2. If `skill/telegram-notifier/config.local.env` is missing or incomplete, read `references/setup-telegram.md` and complete that setup flow.
3. Read `assets/message-template.md` and use that exact plain-text structure.
4. Build a minimal reminder using:
    - current project name
    - current session id
    - a very short task label
    - the fixed completion wording already present in the template
    - task start time
5. Keep the final message to a single concise line whenever possible.
6. Run `python3 skill/telegram-notifier/scripts/send_telegram.py --config skill/telegram-notifier/config.local.env "<final message>"`.
7. If sending fails, note it briefly for yourself and continue with the normal final reply.
